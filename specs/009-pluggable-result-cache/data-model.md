...
| `fallbackValue`             | This generator instance's config, if set                                                                                                                                                                                        | `GroundedGenerator` selects between two different system-prompt variants based on whether a fallback is configured, and both it and `GroundedExtractor` return `fallbackValue` verbatim as part of the result when the pipeline falls back — so it is output-affecting for those two generators and included in their key (code review, PR #19) |
...
`providerAdapter`, `langchainModel`, `maxContextTokens`, `onCall`, `onResult`, `onError`,
`cache` itself. `GroundedEnricher`/`GroundedComposer` never branch on `fallbackValue` in
their pipeline logic, so it is not part of their key input.
