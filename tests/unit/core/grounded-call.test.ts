...
describe('GroundedCall construction', () => {
  ...
  it('throws immediately when model is an empty string and no client is provided', () => {
    process.env['OPENAI_API_KEY'] = 'test-key';
    expect(() => new TestableGroundedCall({ fallbackValue: 'sorry', model: '   ' })).toThrow(
      /model/i
    );
  });

  it('throws when no apiKey and OPENAI_API_KEY is unset', () => {
    expect(() => new TestableGroundedCall({ fallbackValue: 'sorry' })).toThrow(
      /apiKey|OPENAI_API_KEY/i
    );
  });
...
describe('GroundedCall system prompt composition (identity/rules)', () => {
  ...
  it("appends the developer's rules after the base prompt", () => {
    ...
    const prompt = call.getSystemPrompt('BASE INSTRUCTIONS');
    expect(prompt.indexOf('BASE INSTRUCTIONS')).toBeLessThan(
      prompt.indexOf('Always respond in a formal tone.')
    );
  });
...
describe('GroundedCall system prompt composition (tone, 004-behavioral-tone-field)', () => {
  ...
  it("appends the developer's tone after the base prompt when configured alone", () => {
    ...
    expect(prompt.indexOf('BASE INSTRUCTIONS')).toBeLessThan(
      prompt.indexOf('Seja empático e gentil.')
    );
  });
